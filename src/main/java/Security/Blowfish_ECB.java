...
public class Blowfish_ECB {

    /** גודל בלוק ההצפנה בבייטים (64 ביט) */
    ...
    /**
     * מערך P ההתחלתי - משתמש כ-subkey ומעודכן לפי המפתח בזמן ה-Key Expansion.
     * כולל 18 ערכים שמחולקים ל-P-array בן 16 סבבים + שני ערכי pre/post-processing.
     */
    private final static int[] P_INIT = {
    ...
    /**
     * טבלאות ה-S ההתחלתיות (S-boxes) – ארבע טבלאות של 256 ערכים כל אחת,
     * משמשות בפונקציית ה-F של Blowfish. מאופיינות בפיזור גבוה להגברת הבלבול (confusion).
     */
    private final static int[][] S_INIT = {
    ...
    private void init() {
        ...
        for (int i = 0; i < 4; i++) {
            System.arraycopy(S_INIT[i], 0, S[i], 0, 256);
        }
    }
    ...
    public void keyExpansion(byte[] key) {
        ...
        // שלב 1: XOR בין המפתח לבין ה-P-array
        for (int i = 0; i < 18; i++) {
            int data = 0;
            for (int k = 0; k < 4; k++) {
                data = (data << 8) | (key[j] & 0xFF);  // איגוד 4 בתים ל-int
                j = (j + 1) % keyLen;  // לולאה על המפתח
            }
            P[i] ^= data;
        }

        // שלב 2: הצפנת בלוקים של אפס והזנה חזרה ל-P-array
        int left = 0;
        int right = 0;
        for (int i = 0; i < 18; i += 2) {
            int[] res = encryptBlock(left, right);
            left = res[0];
            right = res[1];
            P[i] = left;
            P[i + 1] = right;
        ...
        // שלב 3: אותו תהליך עבור כל ה-SBoxes
        for (int i = 0; i < 4; i++) {
    ...
    public byte[] encrypt(byte[] plainText) {
        ...
        for (int i = 0; i < paddedBytes.length; i += BLOCK_SIZE) {
            int left = bytesToInt(paddedBytes, i);
            int right = bytesToInt(paddedBytes, i + 4);
            int[] encrypted = encryptBlock(left, right);
            intToBytes(encrypted[0], cipherText, i);
            intToBytes(encrypted[1], cipherText, i+4);
        }
    ...
    public byte[] decrypt(byte[] cipherText) {
        ...
        for (int i = 0; i < cipherText.length; i += BLOCK_SIZE) {
            int left = bytesToInt(cipherText, i);
            int right = bytesToInt(cipherText, i + 4);
            int[] decryptedBlock = decryptBlock(left, right);
            intToBytes(decryptedBlock[0], decrypted, i);
            intToBytes(decryptedBlock[1], decrypted, i+4);
        }
